...
/** Get reserved field names that should not be rendered as custom sections */
function getReservedFields() {
  return [
    'version', 'date', 'runtime', 'tags',
    'env', 'scripts', 'breakingChanges', 'deprecated'
  ];
}

/** Render custom fields as markdown sections */
function renderCustomFields(release, lines, knownFields = []) {
  const reserved = getReservedFields();
  const allKnownFields = [...reserved, ...knownFields];
  
  for (const [key, value] of Object.entries(release)) {
    if (allKnownFields.includes(key)) continue;
      ...
      if (s.steps && Array.isArray(s.steps)) {
        for (const step of s.steps) {
          if (typeof step === "string") {
            lines.push(`- ${step}`);
          } else if (step.type && step.contents) {
            lines.push("");
            lines.push("```" + step.type);
            for (const cmd of step.contents) {
              lines.push(cmd);
            }
            lines.push("```");
            lines.push("");
...
export function releaseToMarkdownFull(release) {
  ...
  renderCustomFields(release, lines, ['features', 'bugfixes', 'improvements', 'security']);
...
export function releaseToMarkdownChangelog(current, previous) {
  ...
  // Render custom fields
  renderCustomFields(current, lines, ['features', 'bugfixes', 'improvements', 'security']);
  ...
  if (diff.newScripts.length) {
    lines.push("## New Migration Scripts (compared to previous version)");
    lines.push("");
    for (const s of diff.newScripts) {
      lines.push(`### ${s.title}`);
      lines.push("");
